...
public class ProcessConnectionThread implements Runnable {
	...
	ArrayList<MyProgress> ListProgress;
	String all = "";
	int max = 10;
	...
	public ProcessConnectionThread(StreamConnection connection) {
		...
		ListProgress = new ArrayList<MyProgress>();
		// ****

		for (int k = 0; k < labels.length; k++) {
			ListProgress.add(new MyProgress(labels[k], max));
	...
	public void run() {
		try {
			...
			while (true) {
				...
				byte[] b = new byte[1024]; // this size have to be sufficient,
											// but nothing happens if it's too
											// big, we are going to use only the
											// writes bytes.
				int x = inputStream.read(b); // x give us the number of writed
												// bytes. if x=-1, there is
				...
				int ind = 0;
				for (int i = 0; i < (x); i++) {
					if (j == 8) { // because the numbers for read will be
									// doubles (1 double = 8 bytes)
						j = 0;
						double inputValue = toDouble(aux);
						System.out.print("" + redondeo(inputValue, 1) + " "); // multiply
																				// by
																				// 10
																				// for
																				// have
